<template>
	...
	<section class="px-10 pt-10 lg:pt-30 lg:w-[60%] md:mx-auto">
		<div v-bind="getAOSAttributes({ type: 'fade' })">
			<Media imageSrc="src/assets/images/DSC_0192.jpg" classList="rounded-md" alt="GIVE IMAGE" />
		</div>
	</section>

	<section class="pt-5 px-10 pb-5 lg:w-[80%] md:mx-auto grid grid-cols-1 md:grid-cols-2 place-items-center gap-5">
		<div v-bind="getAOSAttributes()" class="w-full">
          <h1 :class="[show_text ? 'border-b-gray-400 border-b-1' : 'border-b-transparent transition-all ease-in-out', 'my-5 flex items-center text-lg pb-2 uppercase tracking-widest cursor-pointer']" @click="show_text = !show_text">
            <FontAwesomeIcon
          ...
          <p class="my-3 flex flex-col font-winky text-lg" v-if="show_text">
	...
	<section class="px-10 pb-10 lg:w-[80%] md:mx-auto">
		...
		<div class="grid grid-cols-1">
			<div v-bind="getAOSAttributes()" class="w-full" v-for="project in projects" :key="project.id">
				<h1 :class="[project.show ? 'border-b-gray-400 border-b-1' : 'border-b-transparent transition-all ease-in-out', 'my-5 flex items-center text-lg pb-2 uppercase tracking-widest cursor-pointer']" @click="project.show = !project.show">
				<FontAwesomeIcon
					:icon="['fas', 'caret-right']"
					:class="[project.show ? 'rotate-90' : '', 'cursor-pointer mr-2 text-xl']"
				/>
					{{ project.title }} 
				</h1>
				<div v-if="project.show">
					<Media :imageSrc="project.image" classList="rounded-md mt-2" alt="PROJECT IMAGE" />
					<p class="mt-3 mb-2 flex flex-col font-winky text-lg">
						{{ project.description }} 
					</p>
	...
	<section class="px-10 pb-10 lg:w-[80%] md:mx-auto grid grid-cols-1" ref="targetSection">
		<!-- PAYMENT OPTIONS -->
...
const projects = ref([
  {
    id: 1,
    title: "Church Bus",
    description: "Providing safe and reliable transportation for members, especially elderly and youth, ensuring everyone can attend services, events, and outreach programs without difficulty.",
  ...
  },
  {
    ...
    description: "Helping families in need by supplying groceries and meals. Every donation supports the goal of reducing hunger and showing the love of Christ through tangible care.",
    image: "src/assets/images/DSC_0192.jpg",
    show: false
  },
  {
    ...
    description: "Enhancing the worship space with better seating, improved sound systems, and additional rooms for Bible study, youth gatherings, and prayer meetings.",
    image: "src/assets/images/DSC_0192.jpg",
    show: false
  },
  {
	...
	description: "Raising funds to support local and international missions, assisting underprivileged communities, spreading the gospel, and providing essential supplies to those in need.",
	image: "src/assets/images/DSC_0192.jpg", 
	show: false
  }
]);
